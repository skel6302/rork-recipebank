...
struct ImportLinkView: View {
    ...
    private var hero: some View {
        VStack(spacing: 14) {
            ...
            Text("Save a recipe from a video")
                .font(.cookbookSerif(23, weight: .bold))
                .foregroundStyle(Theme.ink)
                .multilineTextAlignment(.center)
            Text("Found a recipe on TikTok, Instagram, or YouTube? Paste the link and we'll read the post and turn it into an editable recipe.")
                .font(.system(size: 14))
                .foregroundStyle(Theme.inkSoft)
                .multilineTextAlignment(.center)
    ...
    private var howItWorks: some View {
        VStack(alignment: .leading, spacing: 10) {
            Label("How it works", systemImage: "lightbulb.fill")
                .font(.system(size: 14, weight: .semibold))
                .foregroundStyle(Theme.spice)
            step("In TikTok or Instagram, tap Share → Copy Link, then paste it here.")
            step("We read the post's caption and pull out the ingredients and steps.")
